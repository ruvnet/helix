...
// Shared SYNTHETIC demo dossier (the "Alex Rivera" persona), generated by the
// helix-demo Rust crate and reused from the desktop console. Not a real person.
let NOW = Date.UTC(2026, 5, 1);
let records = [];
let SYS = [
  { n: "Cardio", v: 72, c: "#34e0c4" }, { n: "Sleep", v: 76, c: "#38bdf8" },
  { n: "Inflam.", v: 80, c: "#9b8cff" }, { n: "Fitness", v: 74, c: "#f6a623" },
];
const SHORT = { cardiometabolic: "Cardio", sleep: "Sleep", inflammation: "Inflam.", fitness: "Fitness" };

async function loadDossier() {
  try {
    const res = await fetch("../ui/demo-dossier.json", { cache: "no-cache" });
    if (!res.ok) throw new Error(`HTTP ${res.status}`);
    const d = await res.json();
    NOW = d.now;
    records = (d.records || []).filter((x) => x.code === "2276-4"); // ferritin thread
    if (Array.isArray(d.subsystems) && d.subsystems.length) {
      SYS = d.subsystems.map((s) => ({ n: SHORT[s.subsystem] || s.subsystem, v: s.value, c: s.color, sub: s }));
    }
  } catch (e) {
...
async function boot() {
  ...
  const keys = ["cardiometabolic", "sleep", "inflammation", "fitness"];
  const subs = SYS.map((s, i) => ({
    subsystem: keys[i], value: s.v, weight: weights[i],
    confidence: s.sub?.confidence ?? 0.85, trend: s.sub?.trend ?? "stable",
    drivers: [{ concept: s.n, points: s.v, trend: s.sub?.trend ?? "stable", source_record: "r" }],
  }));
